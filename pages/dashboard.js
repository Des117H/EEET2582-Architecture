...
import { AppBar, Box, Button, Container, Stack, Toolbar, Typography, CircularProgress, Dialog, Alert, DialogContent, DialogActions, Divider, IconButton, Snackbar } from '@mui/material';
...
export default function dashboard() {
    ...
    const [deleteDocumentImageBucket, setDeleteDocumentImageBucket] = useState("");
    ...
    const onResult = async (documentEnum, isSuccess) => {
        setSnackbarMessage(isSuccess ? SUCCESS_MAP[documentEnum] : ERROR_MAP[documentEnum]);
    ...
    const getDocumentRows = (isConfirmedDocuments, documents) => {
        const zeroStateText = isConfirmedDocuments ? 'No documents to confirm' : 'No past documents';
        const actionEnum = isConfirmedDocuments ? DOCUMENTS_ENUM.confirm : DOCUMENTS_ENUM.edit;
        ...
        <div>
            ...
            <main>
                <Container>
                    ...
                    <Stack direction="row" sx={{ paddingTop: "1.5em" }}>
                        ...
                        <IconButton aria-label="edit" color="secondary" className={styles.addButton}
                            onClick={() => setAction(DOCUMENTS_ENUM.add)}>
                            <AddIcon />
                        </IconButton>
                    </Stack>
                    {getDocumentRows(false, ocrFeatureFlag ? pastDocuments : allDocuments)}
                </Container>
                <ExpenseDialog key={action} document={updateDocument}
                    action={action}
                ...
                <Dialog open={action === DOCUMENTS_ENUM.delete} onClose={resetDelete}>
                    <Typography variant="h4" className={styles.title}>DELETE DOCUMENT</Typography>
                    <DialogContent>
                        <Alert severity="error">This will permanently delete your document!</Alert>
                    </DialogContent>
